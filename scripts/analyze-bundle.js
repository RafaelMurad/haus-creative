...
function analyzeBundle() {
  ...
  if (!fs.existsSync(buildPath)) {
    log('❌ No .next directory found. Please run "npm run build" first.', 'red')
    return
  }
  ...
  try {
    ...
    if (fs.existsSync(chunks)) {
      ...
      if (jsFiles.length > 10) {
        ...
        log(`  ... and ${remaining.length} more files (${Math.round(remainingSize / 1024)} KB)`, 'blue')
      }
      
      log(`\n📈 Total JS Bundle Size: ${Math.round(totalSize / 1024)} KB`, 'bright')
...
function analyzeDependencies(packagePath) {
  ...
  try {
    ...
    const heavyDeps = [
      'gsap', 'react-window', 'next', '@next/bundle-analyzer'
    ]
    ...
    const foundCandidates = treeShakingCandidates.filter(dep => dependencies[dep])
    ...
    log(`\n📊 Total dependencies: ${Object.keys(dependencies).length}`, 'green')
