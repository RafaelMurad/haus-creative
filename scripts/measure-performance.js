...
function calculatePerformanceScore(chunks) {
  const metrics = {
    ...
    vendor: chunks
      ...
      .reduce((sum, c) => sum + c.size, 0),
    gsap: chunks
      .filter((c) => c.name.includes("gsap"))
      .reduce((sum, c) => sum + c.size, 0),
    polyfills: chunks
      .filter((c) => c.name.includes("polyfills"))
      .reduce((sum, c) => sum + c.size, 0),
  };
  ...
  Object.keys(PERFORMANCE_BUDGETS).forEach((key) => {
    if (metrics[key] !== undefined) {
      ...
      const score = Math.max(
        0,
        Math.min(100, ((budget - actual) / budget) * 100 + 50)
      );
      scores[key] = {
        actual,
        budget,
...
function logPerformanceMetrics(results) {
  ...
  if (fs.existsSync(PERFORMANCE_LOG_FILE)) {
    try {
      performanceLog = JSON.parse(
        fs.readFileSync(PERFORMANCE_LOG_FILE, "utf8")
      );
    } catch (e) {
      performanceLog = [];
    }
...
function displayPerformanceDashboard(report) {
  ...
  Object.entries(report.metrics).forEach(([key, size]) => {
    if (size > 0) {
      ...
      console.log(
        `${emoji} ${key.padEnd(12)}: ${size.toString().padStart(4)} KB`
      );
    }
  });
  ...
  Object.entries(report.scores).forEach(([key, data]) => {
    ...
    console.log(
      `${emoji} ${key.padEnd(12)}: ${data.actual}KB / ${
        data.budget
      }KB (${percentage}%)`
    );
  });
  ...
  // Overall score
  const overallScore =
    Object.values(report.scores).reduce((sum, s) => sum + s.score, 0) /
    Object.keys(report.scores).length;
  console.log(
  ...
  if (report.metrics.totalJS > PERFORMANCE_BUDGETS.firstLoadJS) {
    recommendations.push(
      "• Further reduce first load JS with more aggressive code splitting"
    );
  }
  ...
  if (recommendations.length === 0) {
    console.log(
      "🎉 All performance targets met! Consider monitoring with Lighthouse."
    );
  } else {
