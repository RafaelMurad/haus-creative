...
export default function PerformanceMonitor({ children }: PerformanceMonitorProps) {
  const { performanceScore, isMonitoring, startMonitoring } = usePerformanceMonitoring()
    ...
    if (process.env.NODE_ENV === 'development' && isMonitoring) {
      ...
      if (!indicator) {
        ...
        perfDiv.style.cssText = `
          ...
          top: 10px;
          right: 10px;
          background: ${performanceScore >= 90 ? '#10b981' : performanceScore >= 70 ? '#f59e0b' : '#ef4444'};
          color: white;
          padding: 8px 12px;
          border-radius: 4px;
        ...
        indicator.style.background = performanceScore >= 90 ? '#10b981' : performanceScore >= 70 ? '#f59e0b' : '#ef4444'
...
export function PerformanceDebugger() {
  const { metrics, performanceScore, clearMetrics, exportMetrics } = usePerformanceMonitoring()
      ...
      {metrics.length > 0 && (
        <div style={{ marginTop: '10px', maxHeight: '200px', overflow: 'auto' }}>
          ...
          {metrics.slice(-5).map((metric, index) => (
            <div key={index} style={{ 
              padding: '2px 0',
              color: metric.rating === 'good' ? '#10b981' : metric.rating === 'needs-improvement' ? '#f59e0b' : '#ef4444'
            }}>
