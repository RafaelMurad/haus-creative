...
const DynamicGallery = ({ galleries, className = "" }: DynamicGalleryProps) => {
  const [useDynamic, setUseDynamic] = useState(true);

  useEffect(() => {
    // Calculate total items across all galleries
    const totalItems = galleries.reduce(
    ...
    if (process.env.NODE_ENV === "development") {
      console.log(
        `[DynamicGallery] Total items: ${totalItems}, Using dynamic loading: ${shouldUseDynamic}`
      );
    }
  }, [galleries]);
  ...
  if (!useDynamic) {
    return <GalleryStatic galleries={galleries} className={className} />;
  }

  // Use dynamic component for large galleries or slower devices
  return <Gallery galleries={galleries} className={className} />;
};
