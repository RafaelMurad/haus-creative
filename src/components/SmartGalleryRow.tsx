...
const SmartGalleryRow = ({ gallery }: SmartGalleryRowProps) => {
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === "development") {
      console.log(
        `[SmartGalleryRow] Gallery ${gallery.id} items: ${gallery.items.length}, Using dynamic: ${shouldUseDynamic}`
      );
    }
  }, [gallery]);
  ...
  if (!useDynamic) {
    return <GalleryRowStatic gallery={gallery} />;
  }

  // Use dynamic component for large galleries
  return <GalleryRowDynamic gallery={gallery} />;
};
